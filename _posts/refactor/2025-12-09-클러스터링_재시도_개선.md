...
### 3. 재시도 방식
기존의 경우 고정 지수 백오프 방식이므로 분산 환경에서 요청이 겹칠 경우 `외부 voyage api 서버에 부하가 가중될 위험`이 있습니다. 
...
따라서 재시도가 많아질 경우 `OOM`의 위험이 있고, 만약 서버가 죽을 경우 예약된 재시도가 모두 사라지게 됩니다.
...
즉시 `실행 가능한 메세지를 보관`하는 자료구조이며, Redis의 LIST를 활용합니다. Delayed Queue에서 백오프 시간이 지난 메세지가 RPUSH로 추가되면, Consumer 스레드가 LPOP으로 메세지를 꺼내어 처리합니다. 
LPOP은 원자적 연산으로, 여러 Consumer가 동시에 대기하고 있어도 하나의 메세지는 정확히 하나의 Consumer에게만 전달되어 동시성 문제를 방지할 수 있습니다.

#### Processing Queue(ZSET)

`처리 중인 메세지를 백업`하는 자료구조이며, Redis의 ZSET을 활용합니다. ZSET의 score에는 visibilityTimeout 만료 시각을 할당하고, member에는 메세지 내용을 할당합니다. Consumer가 LPOP으로 메세지를 꺼내는 즉시
...
5회 재시도가 모두 `실패한 메세지를 보관`하는 자료구조이며, Redis의 LIST를 활용합니다. 재시도 횟수를 초과한 메세지가 RPUSH로 추가됩니다. 이 Queue의 메세지는
...
### ✅ OutBox 적용하기
#### Outbox Table
![img.png](/assets/posts/img_140.png)
...
public void pollOutboxAndRetry() {
  retryPendingOutboxes();
  recoverStuckOutboxes();
}

private void retryPendingOutboxes() {
  voyageRetryOutboxRepository.findAllByStatus(OutboxStatus.PENDING)
    .forEach(outbox -> {
      try {
        applicationEventPublisher.publishEvent(
          VoyageRetryOutboxCreatedEvent.of(outbox.getFeedbackId())
        );
      } catch (Exception e) {
        log.error("폴링 실패: feedbackId={}", outbox.getFeedbackId(), e);
      }
    });
}

private void recoverStuckOutboxes() {
  LocalDateTime threshold = LocalDateTime.now().minusMinutes(STUCK_THRESHOLD_MINUTES);
  voyageRetryOutboxRepository.findStuckOutboxes(threshold)
    .forEach(outbox -> outbox.updateStatus(OutboxStatus.PENDING));
}
```
- `5분` 주기로 Outbox를 폴링하여 Redis 발송 실패 메시지를 재발송 합니다.
- 재발송 성공 시 Outbox의 해당 메세지를 삭제합니다.
- 메세지 발송 처리(ENQUEUED) 후 `10분` 이상 ENQUEUED 상태로 있을 경우,`stuck 메시지`로 판단하고 이를 다시 PENDING 상태로 복구합니다.
