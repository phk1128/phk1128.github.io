...
JPA 단건 INSERT에서 JDBC Batch INSERT로 전환했습니다. 1000건 단위 chunk로 `item_mst`, `item_uom_dtl`, `item_barcode_dtl` 3개 테이블을 일괄 저장합니다.

```
Phase 1.5 검증: SELECT 1회 (전체 1,000건 한번에)

Phase 2 INSERT:
  chunk 1 (1000건) → INSERT item_mst + UPDATE item_cd + INSERT item_uom_dtl + INSERT item_barcode_dtl = 4 쿼리
  chunk 2 (1000건) → 4 쿼리
  ...
  chunk 10 (1000건) → 4 쿼리
```
