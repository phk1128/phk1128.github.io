...
결과적으로 `외부 API 호출 횟수 -> 최대 1/100로 감소`하였고, 사용자 요청과 무관하게 최대 3회 까지 `백그라운드에서 안정적으로 클러스터링`이 수행됩니다.
...
feedback_embedding_cluster 테이블에 레코드가 점점 쌓일때마다 실패 레코드 조회 시 성능이 비례해서 떨어지기 때문입니다.
...
2시간 마다 다음과 위와 같은 작업을 하게 됩니다.`분산환경`인 점을 고려해서 `스케쥴러 시작시 shedLock`을 걸어줍니다. 또한 여기서 각 벌크연산의 트랜잭션 A,B로 분리되어 있는데 그 이유는 다음과 같습니다.
...
## Todo
- feedback 큐테이블 SUCCESS 상태 레코드 제거 스케쥴러 구현
