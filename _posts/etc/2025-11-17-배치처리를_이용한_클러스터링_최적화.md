...
## 🎬 Intro
> AI 클러스터링을 실시간 비동기 이벤트 처리에서 2시간 배치로 전환하는 과정을 정리해봅니다.

## 결론
결론부터 말씀드리면 기존 비동기 이벤트로 클러스터링 되던 로직을 스케쥴러 + 배치처리를 통해 성능을 개선하였습니다.

구체적으로 `POST 요청 1번당 외부 API 호출 1회` 되던 부분을 `2시간 주기 배치로 최대 100건씩 묶어 처리`하도록 변경했습니다.

결과적으로 `외부 API 호출 횟수 -> 최대 1/100로 감소`하였고, `약 7초이상의 처리 지연`을 `0.06초`로 개선하였습니다.

### 현재 상황
- POST 요청 1번 당(피드백 작성) 비동기 이벤트 형식으로 벡터 계산을 위한 외부 API 호출

### 현재 상황이 왜 문제가 되는가?
AI 클러스터링하는 기능은 비슷한 피드백을 군집으로 묶어주는 역할을 합니다. 따라서 통계의 의미에 가까운 상황 입니다.
해당 기능에 대한 사용자의 피드백은 `실시간성이 필요 없다.` 가 대부분 이었습니다. 따라서 실시간성이 필요 없다면 굳이 비동기 이벤트로 로직을 처리할 이유가 없습니다.

![img.png](/assets/posts/img_101.png)
![img.png](/assets/posts/img_115.png)

이처럼 임베딩 벡터 추출을 위한 외부 API 호출이 2초 정도 소요되기 때문에 이를 트랜잭션 내부에서 사용한다면, 스레드 고갈 문제의 위험이 있습니다.
또한 갑작스럽게 피드백이 많이 작성된다면, 위와 같이 응답시간이 점점 느려지는 병목이 생기게 됩니다.
...
- feedback_embedding_cluster : 클러스터링 성공 시 피드백 데이터가 저장되는 테이블
- embedding_cluster : 대표 문구(label)를 저장하는 테이블

기존에는 클러스터링 성공 시에만 feedback_embedding_cluster 테이블에 저장되는 형식이었습니다.
여기서 문제는 클러스터링이 실패 시 데이터를 모아두지 않고, 로그만 남는다는 점이었습니다. 따라서 이를 해결하기 위해 아래와 같이 2가지 선택지가 있었습니다.

1. 클러스터링 실패 시에도 feedback_embedding_cluster 테이블에 저장, 성공/실패 플래그 컬럼 추가

2. Queue 테이블을 추가하여 성공/실패 클러스터링 데이터 관리

결과적으로 2번 방법을 선택했습니다. 그 이유는 1번의 경우 기존 테이블 스키마 변경이 필요했고, 더해서
feedback_embedding_cluster 테이블에 레코드가 점점 쌓일때마다 실패 레코드 조회 성능이 비례해서 떨어지기 때문입니다.

##### After
![img.png](/assets/posts/img_107.png)

Queue 테이블의 경우 불필요한 참조를 방지하기 위해 클러스터링에 필요한 데이터들로 `반정규화` 하였습니다. 
배치 및 재시도의 기준이 되는 컬럼은 status, retry_count 2가지 입니다.
...
- status 
  ...
  - FAIL : 배치 처리 실패
- retry_count 
  - 최대 3까지 카운팅, 3번째에도 실패 시 status FAIL 처리

추가적으로 SUCCESS 상태의 레코드는 주기적으로 지워주고, FAIL은 개발자가 수동으로 처리해주면 테이블을 가볍게 유지할 수 있습니다.
...
#### 스케쥴러 주기 = 2시간
우테코 코치님들이 매 강의 마다 저희 서비스를 사용하고 계시기 때문에, 강의의 평균 시간을 고려해서 스케쥴러 주기를 2시간으로 설정하였습니다.

#### 배치 단위 = 100개
![img.png](/assets/posts/img_106.png)
[voyage ai 공식 문서](https://docs.voyageai.com/docs/rate-limits)에 따르면 저희 서비스가 사용하고 있는 voyage-3.5 모델의 경우 limit이 TPM = 8,000,000 / RPM = 2,000 입니다.
TPM은 분당 요청할 수 있는 토큰 수, RPM은 분당 요청할 수 있는 요청 수를 의미합니다. 

- 한글 1글자 당 3개의 토큰 사용
- 매 강의시 100글자 이내의 피드백이 50개 생성 됨

위 상황을 고려했을때 50개의 피드백을 요청한다면, 3 * 100 * 50 = 15,000 토큰이 사용 됩니다. 만약 강의가 10개라고 가정해도 TPM은 여유로운 상황입니다.
따라서 서비 메모리와 RPM을 고려해서 배치 단위는 100개로 설정하였습니다.

### ✅ 스케쥴러 + 배치 로직 구현

![img.png](/assets/posts/img_112.png)

2시간 마다 위와 같은 작업을 하게 됩니다.`분산환경`인 점을 고려해서 `스케쥴러 시작시 shedLock`을 걸어줍니다. 또한 여기서 각 벌크연산의 트랜잭션 A,B로 분리되어 있는데 그 이유는 다음과 같습니다.

1. FeedbackEmbeddingCluster가 EmbeddingCluster를 참조 하므로 EmbeddingCluster 벌크 연산이 커밋이 되어야함 
2. FeedbackEmbeddingCluster 처리 과정에서 외부 API를 사용하므로 `EmbeddingCluster 벌크 연산이 실패 하더라도 롤백 되면 안됌`

그리고 1번 상황의 경우는 JDBC에는 영속성 컨텍스트가 없어 벌크 연산 후 객체에 id를 할당하는 방식으로 하여 EmbeddingCluster, FeedbackEmbeddingCluster를
순차적으로 처리하였습니다. 구체적인 코드를 보자면 다음과 같습니다.
...
public class FeedbackClusteringBatchScheduler {
    ...
    public void clusterUnclusteredFeedbacks() {
        ...
        while (true) {
            ...
            log.info("[클러스터링 배치] {}번째 배치 처리 시작 - {}개", batchCount, queues.size());
            ...
            log.info("[클러스터링 배치] {}번째 배치 완료 - 성공: {}개, 실패: {}개",
                    batchCount, processed, queues.size() - processed);

            batchCount++;
        }
        log.info("[클러스터링 배치] 전체 완료 - 총 성공: {}개, 총 실패: {}개", totalProcessed, totalFailed);
    }
    ...
    private int processAllOrganizations(Map<UUID, List<FeedbackClusteringQueue>> queuesByOrg) {
        ...
        for (Map.Entry<UUID, List<FeedbackClusteringQueue>> entry : queuesByOrg.entrySet()) {
            ...
            try {
                ...
                log.error("[클러스터링 배치] 조직 {} 처리 실패", orgUuid, e);
            }
        }
    ...
    private int processOrganizationQueues(List<FeedbackClusteringQueue> queues) {
        ...
        for (int i = 0; i < queues.size(); i += BATCH_SIZE) {
            ...
            try {
                ...
                log.error("[클러스터링 배치] 배치 처리 실패 (index: {}-{})", i, endIndex, e);
            }
        }
...
### ✅ 재시도 가능한 안정적인 클러스터링 환경 구축

#### Before
실패 → 로그만 남김 → 끝 → 해당 피드백은 영원히 클러스터링 안 됨

#### After
실패 → 큐에 retry_count 카운팅 → retry_count < 3일 경우 다음 배치에서 자동 재시도 

## 마무리 하며
이번 리팩토링은 `성능 개선`에 집중하였습니다. 이를 위해 `스케줄러 + 배치 처리`, `JDBC 벌크 연산`을 활용해보았고, 그 과정에서 `shedLock` 개념도 학습해볼 수 있었습니다.

완벽한 설계는 아니지만, 팀 내에서 고민이었던 `병목 현상`과 `재시도 처리`를 한번에 해결할 수 있었어서 뿌듯했습니다.
